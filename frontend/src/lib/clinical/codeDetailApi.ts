...
function unwrap<T>(body: ApiEnvelope<T> | T): T {
  if (body && typeof body === "object" && ("data" in (body as any) || "result" in (body as any))) {
...
export async function fetchCodeDetailOptions(group: "DISEASE" | "DRUG" | "TREAT"): Promise<CodeDetailOption[]> {
