...
import { TREATMENT_RESULT_PROGRESS_STATUS_OPTIONS } from "./treatmentResultDisplay";
...
type TreatmentResultSearchProps = {
  loading?: boolean;
  onSearch: (criteria: TreatmentResultSearchCriteria) => void;
  onReset: () => void;
};

const DEFAULT_SEARCH_TYPE: TreatmentResultSearchType = "patientName";

const SEARCH_TYPE_OPTIONS: Array<{
  value: TreatmentResultSearchType;
  label: string;
}> = [
  ...
  return (
    ...
    >
      <FormControl size="small" sx={{ minWidth: 140 }}>
        <InputLabel id="treatment-result-search-type-label">
          {SEARCH_TYPE_LABEL}
        </InputLabel>
        <Select
          labelId="treatment-result-search-type-label"
          label={SEARCH_TYPE_LABEL}
          value={searchType}
          onChange={(event) =>
            resetSearchFields(
              event.target.value as TreatmentResultSearchType
            )
          }
        >
      ...
      {searchType === "treatmentAt" ? (
        ...
        <FormControl size="small" sx={{ minWidth: 180 }}>
          ...
          <Select
            labelId="treatment-result-search-value-label"
        ...
        <FormControl size="small" sx={{ minWidth: 160 }}>
          <InputLabel id="treatment-result-progress-status-label">
            {PROGRESS_STATUS_LABEL}
          </InputLabel>
          <Select
            labelId="treatment-result-progress-status-label"
